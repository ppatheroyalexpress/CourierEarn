...
export default function DeliveryEntryPage() {
  ...
  const save = async () => {
    ...
    if (total <= 0) {
      setErrorMessage("Please enter at least one delivery.");
      return;
    }
    setSaving(true);
    try {
      ...
      if (error) {
        ...
        setSuccessMessage("Saved delivery entry");
        setCashCollect(0);
        setNotCashCollect(0);
        setEc(0);
      }
    ...
    <main className="min-h-screen bg-zinc-50 dark:bg-black p-4 md:p-8">
      <div className="max-w-xl mx-auto">
        <header className="flex justify-between items-center mb-10">
          ...
          <h1 className="text-xl font-bold">Delivery Entry</h1>
          <button
            onClick={save}
        ...
        <div className="space-y-6 bg-white dark:bg-zinc-900 p-8 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-sm mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
            <div>
              <label className="text-xs font-black uppercase tracking-widest text-zinc-400 mb-2 block" title="Number of Cash Collect deliveries">Cash Collect</label>
              <input
                type="text"
                inputMode="numeric"
                pattern="[0-9]*"
                ...
                onChange={(e) => {
                  ...
                  if (val === "" || /^\d+$/.test(val)) {
                    setCashCollect(val === "" ? 0 : Number(val));
                  }
                }}
              />
            </div>
            <div>
              <label className="text-xs font-black uppercase tracking-widest text-zinc-400 mb-2 block" title="Number of Not Cash Collect deliveries">Not Cash</label>
              <input
                type="text"
                inputMode="numeric"
                pattern="[0-9]*"
                ...
                onChange={(e) => {
                  const val = e.target.value;
                  if (val === "" || /^\d+$/.test(val)) {
                    setNotCashCollect(val === "" ? 0 : Number(val));
                  }
                }}
              />
            </div>
            <div>
              <label className="text-xs font-black uppercase tracking-widest text-zinc-400 mb-2 block" title="Number of EC deliveries">EC</label>
              <input
                type="text"
                inputMode="numeric"
                pattern="[0-9]*"
        ...
        <div className="p-8 rounded-[2rem] bg-black dark:bg-white text-center shadow-xl shadow-black/10">
          <div className="text-xs font-black uppercase tracking-widest text-zinc-500 dark:text-zinc-400 mb-1">Total Deliveries</div>
